...
LABWARE_DEF_JSON = """{"ordering":[["A1"],["A2"],["A3"]],"brand":{"brand":"USCtrayvials","brandId":["ThreeJarTray"]},"metadata":{"displayName":"USCtrayvials 3 Reservoir 60000 µL","displayCategory":"reservoir","displayVolumeUnits":"µL","tags":[]},"dimensions":{"xDimension":127.4,"yDimension":85.4,"zDimension":87},"wells":{"A1":{"depth":76,"totalLiquidVolume":60000,"shape":"circular","diameter":28,"x":25.7,"y":59.7,"z":14},"A2":{"depth":76,"totalLiquidVolume":60000,"shape":"circular","diameter":28,"x":63.7,"y":25.7,"z":14},"A3":{"depth":76,"totalLiquidVolume":60000,"shape":"circular","diameter":28,"x":101.7,"y":59.7,"z":14}},"groups":[{"metadata":{"wellBottomShape":"flat"},"wells":["A1","A2","A3"]}],"parameters":{"format":"irregular","quirks":[],"isTiprack":false,"isMagneticModuleCompatible":false,"loadName":"usctrayvials_3_reservoir_60000ul"},"namespace":"custom_beta","version":1,"schemaVersion":2,"cornerOffsetFromSlot":{"x":0,"y":0,"z":0}}"""
...
def run(protocol: protocol_api.ProtocolContext):
    ...
    if(PIPETTE_NAME == 'p20_single_gen2' or PIPETTE_NAME == 'p300_single_gen2' or PIPETTE_NAME == 'p1000_single_gen2' or PIPETTE_NAME == 'p50_single' or PIPETTE_NAME == 'p10_single' or PIPETTE_NAME == 'p300_single' or PIPETTE_NAME == 'p1000_single'):
        if(total > 1):
            ...
            #second to last well
            well = test_labware.well(last_well-2)
            all_4_edges = [
                [well._from_center_cartesian(x=-1, y=0, z=1), 'left'],
                [well._from_center_cartesian(x=1, y=0, z=1), 'right'],
                [well._from_center_cartesian(x=0, y=-1, z=1), 'front'],
